...
#define SPATIAL_SE3_TYPEDEF_ARG(policy)              \
  typedef typename traitsSE3<policy>::Vector3 Vector3; \
  typedef typename traitsSE3<policy>::Vector4 Vector4; \
  typedef typename traitsSE3<policy>::Vector6 Vector6; \
  typedef typename traitsSE3<policy>::Matrix3 Matrix3; \
  typedef typename traitsSE3<policy>::Matrix4 Matrix4; \
  typedef typename traitsSE3<policy>::Matrix6 Matrix6; \
  typedef typename traitsSE3<policy>::Angular_t Angular_t; \
  typedef typename traitsSE3<policy>::Linear_t Linear_t; \
  typedef typename traitsSE3<policy>::Quaternion_t Quaternion_t; \
  enum {  \
    LINEAR = traitsSE3<policy>::LINEAR,  \
    ANGULAR = traitsSE3<policy>::ANGULAR   \
  }
